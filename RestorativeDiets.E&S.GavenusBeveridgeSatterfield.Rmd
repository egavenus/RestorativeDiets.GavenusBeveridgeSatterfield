...
The values for the Percent of Daily Calories from Salmon represent the primary means for anchoring the modelled estimate in precolonial diets. They were sampled from a distribution of the products of: (i) the likely proportion of diets contributed by fished resources,  (ii) proportion of fished resources comprised of finfish, and  (iii) proportion of finfish calories contributed by salmon—each of which was defined using existing literature to reflect, to the extent possible, dietary practices of coastal First Nations preceding Crown-imposed fisheries governance.
...
### I.B.i. Age
The age structure of the contemporary population of coastal First Nations is retrieved from Statistics Canada: Table 17-10-0144-01  Projected population by Indigenous identity, age group, sex, area of residence, provinces and territories, and projection scenario, Canada (x 1,000), using (unprojected) data from 2016, all areas of residence, total indigenous identity, within in BC
...
```
### Generating Profiles Across Range of Restorative Diets
Using the same distributions above, exploring the parameter values that result in different estimates of salmon use rates, to accompany the distribution figure in demonstration paper.

```{r distribution.profiles, warning=FALSE, message=FALSE}
g.person.yr<-as.integer(g.person.yr)
summary(g.person.yr)
profile1<-which(g.person.yr==66118)
profile2<-which(g.person.yr==68030)
profile3<-which(g.person.yr==156274)
profile4<-which(g.person.yr==150856)
profile5<-which(g.person.yr==221082)
profile6<-which(g.person.yr==233536)

kcal.day[c(profile1, profile2, profile3, profile4, profile5, profile6)]
percent.salmon[c(profile1, profile2, profile3, profile4, profile5, profile6)]
g.kcal.harvest[c(profile1, profile2, profile3, profile4, profile5, profile6)]
...
United States v. Washington, 384 F. Supp. 312, 380 (W.D. Wash. February 12, 1974); aff’d 520 F.2d 676 (9th Cir. 1976), cert. denied, 423 U.S. 1086 (1976), at note 151. 
...
Hewes, G. W. (1973). Indian Fisheries Productivity in Pre-Contact Times in the Pacific Salmon Area. Northwest Anthropological Research Notes, 7(2), 133–155.
...
Harper, B. L., & Harris, S. G. (2008). A possible approach for setting a mercury risk-based action level based on tribal fish ingestion rates. Environmental Research, 107, 60–68. https://doi.org/10.1016/j.envres.2007.05.008
...
Satterfield, T., Robertson, L., Pitts, A., & Jacobson, D. (2015). Reasserting ‘Namgis Food Sovereignty aka Three Boats and a Pick Up Truck.
...
Marushka, L., Kenny, T.-A., Batal, M., Cheung, W. W. L., Fediuk, K., Golden, C. D., … Stewart, P. L. De. (2019). Potential impacts of climate-related decline of seafood harvest on nutritional status of coastal First Nations in British Columbia. PLoS ONE, 14(2), 1–24.
...
### Alternative Figure IV
Following conversations where the previous version implied the contemporary sources were comparable, and the gap between catch + diet would raise additional questions and potentially conclusions that are not appropriate considering the contemporary evidence comes from different years, specific geographies, and methods. Therefore, we are trying here an alternative figure with the aim of highlighting the gap between each contemporary evidence and restorative rates.

```{r alt.figureiv, warning=FALSE, message=FALSE}
nested.bubble.gpy.df<-data.frame(source=c("Estimated Historical", "Catch Data", "Allocation", "Dietary Survey"), amount=c(median, median.catchdata, median.allocation, median.dietarysurvey), order.x=c( 1, 1, 1, 1), order.y=c(1.272, 1.08, 1.044, 0.956))

nested.catch.restorative<-nested.bubble.gpy.df %>%
  filter(source=="Catch Data" | source=="Estimated Historical")

nested.catch.restorative.plot<- ggplot(nested.catch.restorative, 
                        aes(x=order.x, y=order.y, colour=source)) +
  geom_point(aes(size=(amount/median)*100)) +
  labs(y= "", 
       x = "") +
  ...
  scale_colour_manual(name="Type of Source",
                      ...
                      breaks=c("Catch Data", "Estimated Historical"),
                      labels=c("Restorative Diet: Median","Catch Data")) +
  guides(colour="none", size="none")+
  ylim(0.5, 2) +
  xlim(0, 2) +
  ...
  theme(axis.text.x=element_blank(),
        axis.text.y=element_blank(),
        text = element_text(size = 16))
nested.catch.restorative.plot
ggsave("nested.catch.restorative.png", nested.catch.restorative.plot, dpi = 700, width=8, height=6)

nested.allocation.restorative<-nested.bubble.gpy.df %>%
  filter(source=="Allocation" | source=="Estimated Historical")
nested.allocation.restorative.plot<- ggplot(nested.allocation.restorative, 
                        aes(x=order.x, y=order.y, colour=source)) +
  geom_point(aes(size=(amount/median)*100)) +
  labs(y= "", 
       x = "") +
  ...
  scale_colour_manual(name="Type of Source",
                      values=c("#333333", "#999999"),
                      breaks=c("Allocation", "Estimated Historical"),
                      labels=c("Restorative Diet: Median","Allocation Data")) +
  guides(colour="none", size="none")+
  ylim(0.5, 2) +
  xlim(0, 2) +
  theme_classic() +
  theme(axis.text.x=element_blank(),
        axis.text.y=element_blank(),
        text = element_text(size = 16))
nested.allocation.restorative.plot
ggsave("nested.allocation.restorative.png", nested.allocation.restorative.plot, dpi = 700, width=8, height=6)

nested.diet.restorative<-nested.bubble.gpy.df %>%
  filter(source=="Dietary Survey" | source=="Estimated Historical")

nested.diet.restorative.plot<- ggplot(nested.diet.restorative, 
                        aes(x=order.x, y=order.y, colour=source)) +
  geom_point(aes(size=(amount/median)*100)) +
  labs(y= "", 
       x = "") +
  ...
  scale_colour_manual(name="Type of Source",
                      values=c("#333333", "#999999"),
                      breaks=c("Dietary Survey", "Estimated Historical"),
                      labels=c("Restorative Diet: Median","Dietary Data")) +
  guides(colour="none", size="none")+
  ylim(0.5, 2) +
  xlim(0, 2) +
  theme_classic() +
  theme(axis.text.x=element_blank(),
        axis.text.y=element_blank(),
        text = element_text(size = 16))
nested.diet.restorative.plot
ggsave("nested.diet.restorative.png", nested.diet.restorative.plot, dpi = 700, width=8, height=6)
